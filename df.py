...
def drawCumulativeHist(x, ytest, prediction, fname):
    # 创建累积曲线
    # 第一个参数为待绘制的定量数据
    # 第二个参数为划分的区间个数
    # normed参数为是否无量纲化
    ...
    pyplot.plot(x, ytest, linewidth=0.5, color='blue', label="True window")
    pyplot.plot(x, prediction, linewidth=0.5,
                color='red', label="Reg window")
    pyplot.xlabel('Time (s)')
    pyplot.ylabel('window size (\#packet)')
    pyplot.ylim(0, 150)
    pyplot.title('Window Regression')
...
drawCumulativeHist(xx, y_test, XGBpredictions, 'regression')
