...
for item in fr1:
    if (len(item) == 0):
        continue
    (signal, time_busy_b, time_rx_b, time_tx_b, time_scan_b, noise_b, bytes1_b, packets_b, qlen_b, backlog_b,
     drops_b, requeues_b, overlimits_b, una, ret, window) = [float(item[i]) for i in range(1, len(item))]
    X.append([una, ret, signal, time_busy_b, time_tx_b, noise_b,
              packets_b])
    y.append(window)
